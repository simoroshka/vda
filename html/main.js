...
// calculate appropriate token size	and appearance
	setTokenSize = function (numberOfTokens) {
		var k = numberOfTokens;
	...
	setTokenColor = function (amount) {
		...
		graph.vertices.forEach(function(v) { v.changeTokenColor(); });
	}
...
// the classes ---------------------------------------------------------
	...
	//change token's color (in case of scaling) 
	Vertex.prototype.changeTokenColor = function () {
...
// interactions --------------------------------------------------------
	...
	// change the amount of tokens (size)
	changeTokenSize = function (vertex, mouseX, mouseY) {				
				...
				if (options.e.which === 3) {
					...
					if (mousePressed) {
						...
						if (selectedVertex !== null && !shiftPressed) {
							...
							var posX = pointer.x;
							var posY = pointer.y;							
							changeTokenSize(selectedVertex, posX, posY);
						}						
					}	
					// mouse not pressed
					else {
